...
class UI {
    ...
    static bindEvents() {
        ...
        document.getElementById('delete-entry-btn').addEventListener('click', async () => {
            ...
            // Always remove from local storage first so UI updates immediately
            Storage.deleteEntry(id);

            // If Firestore deletion is available and user is authenticated, remove remote copy
            if (window.deleteEntryFromFirestore && window.currentUserId && entry && entry.userId === window.currentUserId) {
                try {
                    await window.deleteEntryFromFirestore(id);
                } catch (err) {
                    console.error('Failed to delete entry from Firestore:', err);
                    // keep going — local deletion already done; optionally notify user
                    this.showToast('Deleted locally but failed to remove from server', 'warning');
                }
            }
